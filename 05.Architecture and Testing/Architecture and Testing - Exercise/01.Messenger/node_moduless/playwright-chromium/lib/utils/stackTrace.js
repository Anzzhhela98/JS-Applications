...
function captureStackTrace() {
    ...
    for (const line of stack.split('\n')) {
        ...
        if (PW_LIB_DIRS.some(libDir => fileName.includes(libDir)))
            continue;
        const isTesting = process.env.PWTEST_CLI_ALLOW_TEST_COMMAND || utils_1.isUnderTest();
        if (isTesting && fileName.includes(path_1.default.join('playwright', 'src')))
            continue;
        if (isTesting && fileName.includes(path_1.default.join('playwright', 'tests', 'config', 'coverage.js')))
            continue;
        frames.push({
            file: fileName,
            line: frame.line,
